Shader "Paint/PaintTransparent" {
	Properties{
		...
		_AlphaDefault("Alpha Default", Range(0, 1)) = 0.0
 ...
 SubShader{ 
        ...
        Blend SrcAlpha OneMinusSrcAlpha // 투명도 블렌딩 설정
...
/*
		...
		//float4 splatMask = smoothstep((_Clip - 0.01) - clipDistHard, (_Clip - 0.01) + clipDistHard, splatSDF.rgb);
       ...
       // o.Alpha = c.a * _Alpha; // 투명도 적용
		// Albedo comes from a texture tinted by color
        float4 MainTex = tex2D(_MainTex, IN.uv_MainTex);
        fixed4 mainColor = MainTex * _Color;

        // Calculate the final color for the main texture without splat colors
        fixed4 finalColor = mainColor;

        // Lerp the color with the splat colors based on the splat mask channels
        finalColor.xyz = lerp(finalColor.xyz, _SplatColor1.xyz, splatMask.x);
        finalColor.xyz = lerp(finalColor.xyz, _SplatColor2.xyz, splatMask.y);
        ...
        o.Metallic = lerp(_Metallic, _SplatMetallic, splatMaskTotal);
        o.Smoothness = lerp(_Glossiness, _SplatGlossiness, splatMaskTotal);
        o.Alpha = finalColor.a * _Alpha; // 투명도 적용
*/
 // Sample splat map texture with offsets
    float4 splatSDF = tex2D(_SplatTex, IN.uv2_SplatTex);
    float4 splatSDFx = tex2D(_SplatTex, IN.uv2_SplatTex + float2(_SplatTex_TexelSize.x, 0));
	...
	//기존 컬러 알파값 수정
	fixed4 color  =  _Color;
	color.a  = _AlphaDefault;

    // Albedo comes from a texture tinted by color
    float4 MainTex = tex2D(_MainTex, IN.uv_MainTex);
    fixed4 c = MainTex * color;

 // Separate alpha channel from splat color
    float4 splatColor1WithAlpha = float4(_SplatColor1.rgb, _Alpha);
    float4 splatColor2WithAlpha = float4(_SplatColor2.rgb, _Alpha);

    // Lerp the color with the splat colors based on the splat mask channels
    float4 mainColor = tex2D(_MainTex, IN.uv_MainTex) * color;
    float4 splatColor1 = lerp(mainColor, splatColor1WithAlpha, splatMask.x);
    float4 splatColor2 = lerp(mainColor, splatColor2WithAlpha, splatMask.y);

    // Choose final color based on splat mask
    float4 finalColor = lerp(splatColor1, splatColor2, splatMask.y);

    o.Albedo = finalColor.rgb;
    o.Normal = tanNormal;
    o.Metallic = lerp(_Metallic, _SplatMetallic, splatMask.y);
    o.Smoothness = lerp(_Glossiness, _SplatGlossiness, splatMask.y);
    o.Alpha = finalColor.a; // Use the alpha from finalColor
	}
